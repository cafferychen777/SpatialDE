...
def _first_derivative(func, x0, step=1.0):
    """Match scipy.misc.derivative for n=1 and its default three-point stencil."""
    return (func(x0 + step) - func(x0 - step)) / (2 * step)


def _second_derivative(func, x0, step=1.0):
    """Match scipy.misc.derivative for n=2 and its default three-point stencil."""
    return (func(x0 + step) - 2 * func(x0) + func(x0 - step)) / step**2
...
def lbfgsb_max_LL(UTy, UT1, S, n, Yvar=None):
    ...
    s2_logdelta = 1. / (_second_derivative(LL_obj, np.log(max_delta)) ** 2)
...
def lengthscale_fits(exp_tab, U, UT1, S, Gower, num=64):
    ...
    for g in tqdm(range(G), leave=False):
        ...
        s2_FSV = _first_derivative(FSV, np.log(max_delta)) ** 2 * s2_logdelta
...
def model_search(X, exp_tab, DE_mll_results, kernel_space=None):
    ...
    ms_results = ms_results.drop(columns=transfer_columns) \
